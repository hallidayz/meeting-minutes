...
/// Minimum speech segment length sent to Whisper (25ms @ 16kHz)
const MIN_VAD_SAMPLES: usize = 400;
...
impl AudioPipeline {
    pub fn new(
        ...
        // Longer redemption bridges natural pauses so utterances aren't fragmented
        ...
        info!(
            "VAD pipeline: separate mic + system paths, redemption={}ms",
            redemption_time
        );
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // STEP 3: Transcribe mic and system audio on separate VAD paths for speaker attribution
                            if mic_window.iter().any(|&s| s.abs() > 0.0005) {
                                match self.mic_vad_processor.process_audio(&mic_window) {
                                    Ok(segments) => self.emit_vad_segments(segments, DeviceType::Microphone),
                                    Err(e) => warn!("⚠️ Mic VAD error: {}", e),
                                }
                            }
                            if sys_window.iter().any(|&s| s.abs() > 0.0005) {
                                match self.sys_vad_processor.process_audio(&sys_window) {
                                    Ok(segments) => self.emit_vad_segments(segments, DeviceType::System),
    ...
    fn emit_vad_segments(
        ...
        for segment in speech_segments {
            ...
            if segment.samples.len() >= MIN_VAD_SAMPLES {
                info!(
                ...
                debug!(
                    "⏭️ Dropping short {:?} segment: {:.1}ms ({} samples)",
                    device_type,
                    duration_ms,
                    segment.samples.len()
                );
